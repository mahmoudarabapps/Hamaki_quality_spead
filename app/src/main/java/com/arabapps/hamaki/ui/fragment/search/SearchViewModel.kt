...
import com.arabapps.hamaki.data.LectureSearchResponse
...
class LectureViewModel : ViewModel() {
    ...
    val loginLiveData: MutableLiveData<LectureSearchResponse> = MutableLiveData()

    fun search(context: Context, text: String) {
        if (!NetworkConnectionHelper.isNetworkConnected(context))
            return
        RetrofitClient.retrofitServices.search(
            SharedHelper.getString(context, SharedHelper.TOKEN).toString(),text
        )
            .enqueue(object : retrofit2.Callback<LectureSearchResponse> {


                override fun onFailure(call: retrofit2.Call<LectureSearchResponse>, t: Throwable) {
                ...
                override fun onResponse(
                    call: retrofit2.Call<LectureSearchResponse>,
