...
import com.arabapps.hamaki.data.LectureByIDResponse
...
class LectureViewModel : ViewModel() {
    ...
    val loginLiveData: MutableLiveData<LectureByIDResponse> = MutableLiveData()

    fun lectureByID(context: Context, id: Int) {
        if (!NetworkConnectionHelper.isNetworkConnected(context))
            return

        if (BuildConfig.DEBUG)
            Log.d(TAG, "lectureByID: "+SharedHelper.getString(context, SharedHelper.TOKEN) +"\n"+id)
        RetrofitClient.retrofitServices.lecture(
            SharedHelper.getString(context, SharedHelper.TOKEN).toString(),id
        )
            .enqueue(object : retrofit2.Callback<LectureByIDResponse> {


                override fun onFailure(call: retrofit2.Call<LectureByIDResponse>, t: Throwable) {
                ...
                override fun onResponse(
                    call: retrofit2.Call<LectureByIDResponse>,
                    response: Response<LectureByIDResponse>
                ) {
